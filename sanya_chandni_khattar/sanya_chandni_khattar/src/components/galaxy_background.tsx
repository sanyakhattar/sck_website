...
function createRandom(seed: number) 
{
  ...
  function starField(count: number, seed: number, maxSize: number)
  {
    ...
    return Array.from({ length: count }, () => 
      ...
      const size = (rand() * maxSize + 0.4).toFixed(2);
      const alpha = (rand() * 0.6 + 0.35).toFixed(2);
      return `radial-gradient(${size}px ${size}px at ${x}% ${y}%, rgba(255,255,255,${alpha}), transparent 100%)`;
    }).join(", ");
  } 
...
const LAYER_NEAR = starField(90, 12345, 1.6);
const LAYER_MID = starField(150, 67890, 1.1);
const LAYER_FAR = starField(250, 24680, 0.7);
...
export function GalaxyBackground({ className }: { className?: string }) {
  return (
    ...
    >
      {/* nebula clouds / the galactic core */}
      <div
        className="galaxy-band absolute -inset-[40%] opacity-70"
        style={{
          backgroundImage: [
      ...
      <div
        className="absolute -inset-[40%] opacity-80 mix-blend-multiply"
        style={{
          backgroundImage: [
      ...
      {/* star layers, each drifting at its own rate */}
      <div className="star-layer absolute inset-0" style={{ backgroundImage: LAYER_FAR, animationDuration: "220s" }} />
      <div className="star-layer absolute inset-0" style={{ backgroundImage: LAYER_MID, animationDuration: "160s" }} />
      <div className="star-layer star-twinkle absolute inset-0" style={{ backgroundImage: LAYER_NEAR, animationDuration: "110s" }} />
